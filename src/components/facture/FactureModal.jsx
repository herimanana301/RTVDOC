...
export default function FactureModal(props) {
    ...
    useEffect(() => {
        FindOneCommande(id, setDatasCommande, setPageInfo, setDatasPrestation, setDatasPayement);
    ...
    useEffect(() => {
        ...
        if (datasPayement) {
            if (datasPayement.attributes.typePayement === 'Partillement-payé') {
                setFormData({
                    datePayement: new Date(datasPayement.attributes.datePayement),
                    avanceMontant: datasPayement.attributes.avance,
                    restePayement: (datasPayement.attributes.montantTotal - datasPayement.attributes.avance),
            ...
            >
                <form onSubmit={(event) => event.preventDefault()}>
                    ...
                    <SimpleGrid cols={1} breakpoints={[{ maxWidth: 'sm', cols: 1 }]}>
                        <Select
                            label="Payement"
                            data={['Partillement-payé', 'Totalement-payé']}
                            onChange={(e) =>
                                setFormData((prevData) => {
                        ...
                        />
                        {FormData.typePayement === 'Partillement-payé' && (
                            <div>
                                <NumberInput
                                    hideControls
                    ...
                    <NumberInput
                        ...
                        readOnly
                    />
                    {datasPayement && datasPayement.attributes.typePayement !== 'Totalement-payé' && (
                        <Group style={{ display: "flex", justifyContent: "space-between" }} mt="md">
                            <Button onClick={submitButtonUpdate} type="submit">Sauvegarder la modification</Button>
                        </Group>
                    )}
                    {!datasPayement && (
                        <Group style={{ display: "flex", justifyContent: "space-between" }} mt="md">
